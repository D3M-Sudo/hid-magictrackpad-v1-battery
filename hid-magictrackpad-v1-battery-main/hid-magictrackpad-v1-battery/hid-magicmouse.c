		...
		struct hid_report *report, u8 *data, int size)
{
	...
	case MOUSE2_REPORT_ID:
		/* Expect 14 bytes of prefix, and N*8 bytes of touch data. */
	...
	case DOUBLE_REPORT_ID:
		/* This report contains two reports, each with a 1-byte length prefix. */
...
static int magicmouse_setup_input(struct input_dev *input, struct hid_device *hdev)
{
	...
	if (input->id.product == USB_DEVICE_ID_APPLE_MAGICMOUSE ||
		...
		if (hdev->vendor == BT_VENDOR_ID_APPLE) {
			if (input->id.version == 0x110) /* Trackpad 2021 */
				input->name = "Apple Inc. Magic Trackpad 2021";
			else if (input->id.version == 0x314) /* Trackpad 2024 */
...
static void magicmouse_battery_timer_tick(struct timer_list *t)
{
	...
	mod_timer(&msc->battery_timer, jiffies + TRACKPAD_V1_BATTERY_TIMEOUT_SEC * HZ);
}
